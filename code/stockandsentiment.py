...
correlation = merged_df['sentiment_vader'].corr(merged_df['Close'])
print("Correlation between sentiment score and Close price:", correlation)
positive_sentiment_returns = merged_df[merged_df['sentiment_vader'] > 0]['Close'].pct_change()

print("Average returns for positive sentiment days:", positive_sentiment_returns.mean())
plt.scatter(merged_df['sentiment_vader'], merged_df['Close'])
...
plt.plot(merged_df['formatted_date'], merged_df['Close'], label='Close Price')
plt.xlabel('formatted_date')
plt.ylabel('Value')
plt.title('Sentiment Score and Close Price Time Series')
plt.legend()
plt.show()
